...
from (select b.name, b.founded, m.name as model from brands as b left join models as m using (brand_id)) as b
...
select b.brand_id from brands as b join models as m using (brand_id) group by b.brand_id having count(model_id) > 5;
